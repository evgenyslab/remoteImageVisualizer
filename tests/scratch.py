...
dataHtml = {
    b'plotly':True,
    b'x': np.random.rand(1, npts).flatten().tolist(),
    b'y': np.random.rand(1, npts).flatten().tolist(),
    b'z': np.random.rand(1, npts).flatten().tolist(),
    b'config': {
        b'mode': b'markers',
        b'marker': {
            b'size': 12,
            b'line': {
                b'color': b'rgba(217, 217, 217, 0.14)',
                b'width': 0.5},
            b'opacity': 0.8},
        b'type': b'scatter3d',
        b'showscale': False
